...
import com.example.demo.api.entities.reservation.ReservationRequestBody;
...
public class ReservationService {
    ...
    public Reservation createReservation(ReservationRequestBody reservationRequestBody) {
        Reservation reservation = new Reservation(UUID.randomUUID(), UUID.fromString(reservationRequestBody.getGuestId()),
                DateTimeFormatManager.getLocalDateTimeFormat(reservationRequestBody.getStartTime()),
                DateTimeFormatManager.getLocalDateTimeFormat(reservationRequestBody.getEndTime()),
                reservationRequestBody.getCapacity(), UUID.fromString(reservationRequestBody.getRestaurantId()), UUID.fromString(reservationRequestBody.getTableId()));
    ...
    public Reservation makeAReservationByCapacityAndDateTime(UUID guestId, String startTime, String endTime, int capacity, String restaurantName) {
        TableUnit tableUnit = restaurantService.getTablesByCapacityDateTimeAndRestaurant(capacity, startTime, restaurantName).get(0);
